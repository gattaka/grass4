...
public interface ContentNodeRepositoryCustom {
    ...
    long countByFilterAndUserAccess(ContentNodeFilterTO filter, Long userId, boolean admin);

    List<ContentNodeOverviewTO> findByFilterAndUserAccess(ContentNodeFilterTO filter, Long userId, boolean admin,
                                                          int offset, int limit, String sortProperty);

    List<String> findTagsByContentId(Long id);

    ContentNodeTO findByIdForDetail(Long contentNodeId);
}
