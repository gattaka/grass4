...
public class DrinkRepositoryCustomImpl implements DrinkRepositoryCustom {
    ...
    public long countBeers(String filter) {
        ...
        QDrink d = QDrink.drink;
        QBeerInfo b = QBeerInfo.beerInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateBeers(filter)).select(d.id).stream()
                .count();
    }
    ...
    public long countBeers(BeerOverviewTO filterTO) {
        ...
        QDrink d = QDrink.drink;
        QBeerInfo b = QBeerInfo.beerInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateBeers(filterTO)).select(d.id)
                .stream().count();
    }
    ...
    public List<BeerOverviewTO> findBeers(BeerOverviewTO filterTO, int offset, int limit, OrderSpecifier<?>[] order) {
        ...
        if (order == null || order.length == 0) order =
                QuerydslUtil.transformOrdering(new String[]{b.brewery.toString(), d.name.toString()},
                        new boolean[]{true, true});
        return query.select(
                        new QBeerOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, b.id, b.brewery, b.ibu,
                                b.degrees, b.category, b.maltType, b.malts, b.hops)).from(d).join(b).on(d.drinkInfo.eq(b.id))
                .where(createPredicateBeers(filterTO)).orderBy(order).fetch();
    }
    ...
    public List<BeerOverviewTO> findBeers(String filter, PageRequest pageable) {
        ...
        return query.select(
                        new QBeerOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, b.id, b.brewery, b.ibu,
                                b.degrees, b.category, b.maltType, b.malts, b.hops)).from(d).join(b).on(d.drinkInfo.eq(b.id))
                .where(createPredicateBeers(filter)).orderBy(
                        QuerydslUtil.transformOrdering(new String[]{b.brewery.toString(), d.name.toString()},
                                new boolean[]{true, true})).fetch();
    }
    ...
    public BeerTO findBeerById(Long id) {
        ...
        QBeerInfo b = QBeerInfo.beerInfo;
        return query.select(
                        new QBeerTO(d.id, d.name, d.type, d.rating, d.image, d.description, d.alcohol, d.country, b.id,
                                b.brewery, b.ibu, b.degrees, b.category, b.maltType, b.malts, b.hops)).from(d).join(b)
                .on(d.drinkInfo.eq(b.id)).where(d.id.eq(id)).fetchOne();
    }
    ...
    public long countRums(RumOverviewTO filterTO) {
        ...
        QDrink d = QDrink.drink;
        QRumInfo b = QRumInfo.rumInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateRums(filterTO)).select(d.id).stream()
                .count();
    }
    ...
    public long countRums(String filter) {
        ...
        QDrink d = QDrink.drink;
        QRumInfo b = QRumInfo.rumInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateRums(filter)).select(d.id).stream()
                .count();
    }
    ...
    public List<RumOverviewTO> findRums(RumOverviewTO filterTO, int offset, int limit, OrderSpecifier<?>[] order) {
        ...
        return query.select(
                        new QRumOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, r.id, r.years, r.rumType))
    ...
    public List<RumOverviewTO> findRums(String filter, PageRequest pageable) {
        ...
        return query.select(
                ...
                .orderBy(QuerydslUtil.transformOrdering(new String[]{d.name.toString()}, new boolean[]{true})).fetch();
    }
    ...
    public RumTO findRumById(Long id) {
        ...
        QRumInfo b = QRumInfo.rumInfo;
        return query.select(
                new QRumTO(d.id, d.name, d.type, d.rating, d.image, d.description, d.alcohol, d.country, b.id, b.years,
                        b.rumType)).from(d).join(b).on(d.drinkInfo.eq(b.id)).where(d.id.eq(id)).fetchOne();
    }
    ...
    public long countWhiskeys(WhiskeyOverviewTO filterTO) {
        ...
        QDrink d = QDrink.drink;
        QWhiskeyInfo b = QWhiskeyInfo.whiskeyInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateWhiskeys(filterTO)).select(d.id)
                .stream().count();
    }
    ...
    public long countWhiskeys(String filter) {
        ...
        QDrink d = QDrink.drink;
        QWhiskeyInfo b = QWhiskeyInfo.whiskeyInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateWhiskeys(filter)).select(d.id)
                .stream().count();
    }
    ...
    public List<WhiskeyOverviewTO> findWhiskeys(WhiskeyOverviewTO filterTO, int offset, int limit,
        ...
        return query.select(new QWhiskeyOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, w.id, w.years,
                        w.whiskeyType)).from(d).join(w).on(d.drinkInfo.eq(w.id)).where(createPredicateWhiskeys(filterTO))
                .orderBy(order).fetch();
    }
    ...
    public List<WhiskeyOverviewTO> findWhiskeys(String filter, PageRequest pageable) {
        ...
        return query.select(new QWhiskeyOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, w.id, w.years,
                        w.whiskeyType)).from(d).join(w).on(d.drinkInfo.eq(w.id)).where(createPredicateWhiskeys(filter))
                .orderBy(QuerydslUtil.transformOrdering(new String[]{d.name.toString()}, new boolean[]{true})).fetch();
    }
    ...
    public WhiskeyTO findWhiskeyById(Long id) {
        ...
        QWhiskeyInfo b = QWhiskeyInfo.whiskeyInfo;
        return query.select(
                new QWhiskeyTO(d.id, d.name, d.type, d.rating, d.image, d.description, d.alcohol, d.country, b.id,
                        b.years, b.whiskeyType)).from(d).join(b).on(d.drinkInfo.eq(b.id)).where(d.id.eq(id)).fetchOne();
    }
    ...
    public long countWines(WineOverviewTO filterTO) {
        ...
        QDrink d = QDrink.drink;
        QWineInfo b = QWineInfo.wineInfo;
        return query.from(d).join(b).on(d.drinkInfo.eq(b.id)).where(createPredicateWines(filterTO)).select(d.id)
                .stream().count();
    }
    ...
    public List<WineOverviewTO> findWines(WineOverviewTO filterTO, int offset, int limit, OrderSpecifier<?>[] order) {
        ...
        if (order == null || order.length == 0) order =
                QuerydslUtil.transformOrdering(new String[]{w.winery.toString(), d.name.toString()},
                        new boolean[]{true, true});

        return query.select(
                        new QWineOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, w.id, w.winery, w.year,
                                w.wineType)).from(d).join(w).on(d.drinkInfo.eq(w.id)).where(createPredicateWines(filterTO))
                .orderBy(order).fetch();
    }
    ...
    public WineTO findWineById(Long id) {
        ...
        QWineInfo b = QWineInfo.wineInfo;
        return query.select(
                        new QWineTO(d.id, d.name, d.type, d.rating, d.image, d.description, d.alcohol, d.country, b.id,
                                b.winery, b.year, b.wineType)).from(d).join(b).on(d.drinkInfo.eq(b.id)).where(d.id.eq(id))
                .fetchOne();
    }
    ...
    public long countOthers(OtherOverviewTO filterTO) {
        ...
        return query.from(d).join(o).on(d.drinkInfo.eq(o.id)).where(createPredicateOthers(filterTO)).select(d.id)
                .stream().count();
    }
    ...
    public List<OtherOverviewTO> findOthers(OtherOverviewTO filterTO, int offset, int limit,
        ...
        return query.select(
                        new QOtherOverviewTO(d.id, d.name, d.type, d.rating, d.alcohol, d.country, o.id, o.ingredient)).from(d)
                .join(o).on(d.drinkInfo.eq(o.id)).where(createPredicateOthers(filterTO)).orderBy(order).fetch();
    }
    ...
    public OtherTO findOtherById(Long id) {
        ...
        QOtherInfo b = QOtherInfo.otherInfo;
        return query.select(
                new QOtherTO(d.id, d.name, d.type, d.rating, d.image, d.description, d.alcohol, d.country, b.id,
                        b.ingredient)).from(d).join(b).on(d.drinkInfo.eq(b.id)).where(d.id.eq(id)).fetchOne();
    }
}