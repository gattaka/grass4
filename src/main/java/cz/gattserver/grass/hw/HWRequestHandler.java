...
public class HWRequestHandler extends AbstractGrassRequestHandler {
    ...
    public HWRequestHandler(@Lazy HWService hwService) {
        this.hwService = hwService;
    }

    @Override
    protected Path getPath(String fileName, HttpServletRequest request) throws FileNotFoundException {
        if (!fileName.matches("/[0-9]+/(icon|img|print3d|doc)/[^/]+")) throw new GrassPageException(400);
        String[] chunks = fileName.split("/");
        Long id = Long.parseLong(chunks[1]);
        String type = chunks[2];
        String name = chunks[3];
        if ("print3d".equals(type)) return hwService.findHWItemPrint3dFilePath(id, name);
        if ("img".equals(type)) return hwService.findHWItemImagesFilePath(id, name);
        if ("icon".equals(type)) try {
