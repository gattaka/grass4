...
public class ScheduledVisitDialog extends EditWebDialog {
	...
	private ScheduledVisitDialog(ScheduledVisitTO originalOrNewTO, ScheduledVisitTO fromToBePlannedTO,
		...
		institutionComboBox.addValueChangeListener(event -> {
			if (event.getValue() != null && recordsComboBox.getValue() == null) {
				Long recordId = medicService.getInstitutionLastRecordId(event.getValue().getId());
				if (recordId != null) {
					binder.getBean().setRecordId(recordId);
					binder.refreshFields();
				}
			}
		});

		recordsComboBox.addValueChangeListener(event -> {
			if (event.getValue() != null && institutionComboBox.getValue() == null) {
				binder.getBean().setInstitutionId(event.getValue().getInstitutionId());
				binder.refreshFields();
			}
		});
