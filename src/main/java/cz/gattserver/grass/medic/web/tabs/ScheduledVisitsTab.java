    ...
    private void prepareGrid(Grid<ScheduledVisitTO> grid, boolean fullTime) {
        ...
        grid.addColumn(new IconRenderer<>(item -> {
            if (item.getState().equals(ScheduledVisitState.MISSED)) {
                Image img = ImageIcon.WARNING_16_ICON.createImage("Zmeškáno");
                img.addClassName(UIUtils.GRID_ICON_CSS_CLASS);
                return img;
            } else {
                if (MedicUtil.isVisitPending(item)) return ImageIcon.CLOCK_16_ICON.createImage("Blíží se");
            }
            return new Span();
        ...
        if (fullTime) grid.addColumn(new LocalDateTimeRenderer<>(to -> to.getDate().atTime(to.getTime()),
        ...
        else grid.addColumn(new LocalDateTimeRenderer<>(to -> to.getDate().atStartOfDay(),
        ...
        grid.addColumn(new TextRenderer<>(to -> to.getInstitution() == null ? "" : to.getInstitution().getName()))
                .setKey("institution").setHeader("Instituce");
