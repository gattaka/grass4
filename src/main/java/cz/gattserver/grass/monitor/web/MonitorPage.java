...
public class MonitorPage extends Div {
    ...
    private String constructUsedTotalFreeDescription(long used, float ratio, long total, long free) {
        return "obsazeno " + humanFormat(used) + " (" + NumberFormat.getIntegerInstance().format(ratio * 100) +
                "%) z " + humanFormat(total) + "; volno " + humanFormat(free);
    }
    ...
    private void createSystemMemoryStatusPart(SystemMemoryMonitorItemTO memoryTO) {
        ...
        switch (memoryTO.getMonitorState()) {
            ...
            case ERROR:
            default:
                memoryStatusTableLayout.newRow().add(new ErrorMonitorStateLabel())
    ...
    private void createJVMThreadsPart(JVMThreadsMonitorItemTO threadsTO) {
        ...
        switch (threadsTO.getMonitorState()) {
            ...
            case ERROR:
            default:
                jvmThreadsTableLayout.newRow().add(new ErrorMonitorStateLabel())
                        .add("JVM thread info není " + "dostupné");
        }
    }
                ...
                jvmMemoryTableLayout.newRow().add(new SuccessMonitorStateLabel())
                        .add(constructProgressMonitor(usedRatio,
                                constructUsedTotalFreeDescription(memoryTO.getUsedMemory(), usedRatio,
                                        memoryTO.getTotalMemory(), memoryTO.getFreeMemory())));
    ...
    private void populateMonitor() {
        ...
        for (Entry<String, Integer> entry : partsAndIntervals.entrySet()) {
            ...
            UI.getCurrent().getPage().executeJs(
                    "window." + TIMEOUTS_JS_ARRAY + ".push(setTimeout(function(){" + js + "}," + 1000 + "));");
        }
