...
public class PGServiceImpl implements PGService {
    ...
    private void processMiniatureImages(Photogallery photogallery, boolean reprocess) throws IOException {
        ...
        if (reprocess) {
            if (Files.exists(miniDirFile)) deleteFileRecursively(miniDirFile);
            if (Files.exists(prevDirFile)) deleteFileRecursively(prevDirFile);
        }
    ...
    private void processSlideshowImages(Photogallery photogallery, boolean reprocess) throws IOException {
        ...
        if (reprocess && Files.exists(slideshowDirFile)) deleteFileRecursively(slideshowDirFile);

        if (!Files.exists(slideshowDirFile)) Files.createDirectories(slideshowDirFile);
    ...
    private Comparator<Path> getComparator() {
        ...
        Comparator<Path> comparator = (p1, p2) -> {
            try {
                ...
                LocalDateTime ldt2 =
                        Files.getLastModifiedTime(p2).toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
            ...
            Stream.concat(miniaturesStream, Stream.concat(previewsStream, otherStream)).skip(skip).limit(limit)
                    .forEach(file -> {
                        ...
                        if (file.startsWith(previewDir)) {
                            ...
                            itemTO.setType(MediaType.IMAGE);
