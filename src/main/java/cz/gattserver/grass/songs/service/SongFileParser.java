...
public class SongFileParser {
    ...
    public static SongTO parseSongInfo(String fileName) {
        ...
        if (authorEnd < 0) throw new IllegalStateException(AUTHOR_ERR);

        String author = string.substring(0, authorEnd).trim();
        if (author.isEmpty()) throw new IllegalStateException(AUTHOR_ERR);
        to.setAuthor(author);
        ...
        if (name.isEmpty()) throw new IllegalStateException(NAME_ERR);
        to.setName(name);

        if (nameEnd > 0) {
            String year = string.substring(nameEnd).trim();
            if (!year.matches("\\([0-9]{4}\\)")) throw new IllegalStateException(YEAR_ERR);
